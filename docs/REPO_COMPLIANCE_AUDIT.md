...
- Supabase access and SQL schema (`src/db/supabase.ts`, `src/db/schema.sql`)
...
8. **DB schema includes required MVP-B tables and key constraints.**
   - `src/db/schema.sql` includes: `accounts`, `entitlements`, `user_configs`, `device_tokens`, `ingest_runs`, `fills_canonical`, `mode_state`, `violations`, `drift_scores`, `webhook_events`.

9. **Financial precision and idempotency constraints exist in schema.**
   - `src/db/schema.sql` uses `NUMERIC(18,8)` for `fills_canonical.price` and `fills_canonical.commission`.
   - `src/db/schema.sql` has `fills_canonical.event_id` primary key and `UNIQUE(device_id,file_hash)` on `ingest_runs` (partial index) and `UNIQUE(token_hash)` on `device_tokens`.

10. **RLS read policies and authenticated write-deny policies are present in schema.**
    - `src/db/schema.sql` enables RLS per table, defines SELECT policies, and denies authenticated writes.
