...
- `occ` = running count of identical `T` encountered so far in that batch using **1-based indexing** (`1, 2, 3, ...`).
...
## 5.5 `drift_scores` stale-state contract
- `drift_scores.state` **MUST** be one of `{OK, DRIFT, DATA_STALE}`.
- `drift_scores.data_stale` **MUST** be a boolean.
- When `data_stale = true`, the system **MUST** hold the last computed state and **MUST** block re-evaluation until new canonical fills are available.
